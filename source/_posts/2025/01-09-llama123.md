...
metaAI 从 helpfulness 和 safety 两个维度衡量人类对机器回答的偏好程度。标注者首先编写 prompt 喂给两个不同参数的模型变体，并按给定的 guidelince 给得到的两个回答打分。
...
为了使 RM 和对话模型“具备相同的知识”，metaAI 将两个 RM 初始化成经过预训练的对话模型的一个 checkpoint。也就是说，RM 采用与对话模型完全相同的架构和训练参数，而仅 **将模型输出头从预测 next-token 头替换为回归头**（就如下面公式展示的）。
...
metaAI 执行 RLHF 时主要用了两个算法： **Proximal Policy Optimization(PPO)** 和 **Rejection Sampling fine-tuning**。前者是一个通用的 RLHF 的范式，后者
