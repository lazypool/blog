...
**Rejection Sampling fine-tuning** 亦即 **拒绝采样微调** 。给定 prompt，它经由具有不同 Temperature 参数的 LLaMA2-chat 得到一组可能的回答。RSF 从这些回答中选取 RM 得分最高的前 N 个样本，并用这些样本对 LLaMA2-Chat 做一次小规模的 SFT ~（这种方法有点像遗传算法）~ 。在 RLHF 的前三次迭代 (v1 to v3) 中，LLaMA2-Chat 只通过 RSF 学习，直到 v4 才引入 PPO 算法。
